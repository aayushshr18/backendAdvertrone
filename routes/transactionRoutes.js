...
router.post("/employee/withdraw", async (req, res) => {
  try {
    const { amount, employee_id, agentId } = req.body;
    ...
    // Calculate the sum of amounts from referred agents
    // let referredAgentsSum = 0;
    if (
      employeeDetail.referred_agent_id &&
      employeeDetail.referred_agent_id.length > 0
    ) {
      const referredAgentAmounts = employeeDetail?.referred_agent_id?.map(
        (agent) => agent.amount
      );

      // const referredAgentAmounts = employeeDetail?.referred_agent_id?.map((agent) => agent.amount) || [];



      referredAgentsSum = referredAgentAmounts.reduce(
    ...
    // if (referredAgentsSum < amount) {
    //   return res.status(400).json({
    //     success: false,
    //     message: "Insufficient balance from referred agents for withdrawal",
    //   });
    // }

    // Perform the withdrawal operation from referred agent amounts

    // You might want to implement the logic here to deduct 'amount' from referred agent sums
    ...
    const transactionHistory = new TransactionHistory({
      ...
      agent_code : employeeDetail.agent_code,
      accessibleBalance: employeeDetail.accessibleBalance, // Store the referred agents sum here
      amount,
      employee_id,
    });
